...
ENCRYPTION_PIPE = ["security", "add-generic-password", "-U", "-a", "{account}", "-s", "mbsync-oauth2-token", "-w"] # ['gpg', '--encrypt', '--recipient', 'jm@jmibanez.com']
DECRYPTION_PIPE = ["security", "find-generic-password", "-a", "{account}", "-s", "mbsync-oauth2-token", "-w"] # ['gpg', '--decrypt']
...
try:
    decrypt_args = [a.format(account=args.account) for a in DECRYPTION_PIPE]
    sub = subprocess.run(decrypt_args, check=True, capture_output=True)
...
def writetokenfile():
    ...
    # if 0o777 & path.stat().st_mode != 0o600:
    #     sys.exit('Token file has unsafe mode. Suggest deleting and starting over.')
    procargs = [a.format(account=args.account) for a in ENCRYPTION_PIPE]
    procargs.append(json.dumps(token).encode())
    sub2 = subprocess.run(procargs, check=True)
